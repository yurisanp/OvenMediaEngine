...
install_ffmpeg()
{
    ...
    if [ "$NVIDIA_NV_CODEC_HWACCELS" = true ] ; then
        ADDI_CFLAGS+="-I/usr/local/cuda/include -gencode arch=compute_52,code=sm_52 "
        ADDI_LDFLAGS="-L/usr/local/cuda/lib64 "
        ADDI_LICENSE+=" --enable-nonfree "
        ADDI_LIBS+=" --enable-cuda-nvcc --disable-cuda-llvm --enable-nvenc --enable-nvdec --enable-ffnvcodec --enable-cuvid "
